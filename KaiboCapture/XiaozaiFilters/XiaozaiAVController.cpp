...
XiaozaiAVController::XiaozaiAVController(LPUNKNOWN pUnk, HRESULT *phr):
CUnknown(NAME("XiaozaiAVController"), pUnk)
{

}

XiaozaiAVController::XiaozaiAVController():
CUnknown(NAME("XiaozaiAVController"), this)
{
}
...
//
STDMETHODIMP XiaozaiAVController::NonDelegatingQueryInterface(REFIID riid, void ** ppv)
{
...
HRESULT STDMETHODCALLTYPE XiaozaiAVController::CleanUp()
{
...
HRESULT XiaozaiAVController::OpenFileorDevice(const BSTR sName, const BSTR argstr, FFMpegInputWapper& theWapper)
{
	...
	pCodecCxt->request_channel_layout = 3; 
...
///
HRESULT STDMETHODCALLTYPE XiaozaiAVController::SetSourceDevice(const BSTR sDevName, const BSTR argstr)
{
	HRESULT res = S_OK;
...
/// Set the source file
///
HRESULT STDMETHODCALLTYPE XiaozaiAVController::SetSourceFileName(const BSTR sFileName)
{
	HRESULT res = S_OK;
...
HRESULT STDMETHODCALLTYPE XiaozaiAVController::SetRTMPServer(const BSTR sURL, const BSTR argstr)
{
...
HRESULT STDMETHODCALLTYPE XiaozaiAVController::SetRecordPath(const BSTR outputPath, const BSTR argstr)
{
	m_sRecordPath = SysAllocString(outputPath);
	m_sRecordArgs = SysAllocString(argstr);
	bool done = m_recordFileWapper.set_options(m_sRecordArgs, 0);

	done = m_recordFileWapper.open_output(OS::StringT2UTF8(m_sRecordPath).c_str(), false);

	if (!m_AudioEncoder.IsOpen())
	{
		m_AudioEncoder.set_options(m_sURLArgs, 0);
		m_AudioEncoder.open_codec(m_recordFileWapper.get_audio_stream_codec());
	}
	...
	{
		m_VideoEncoder.set_options(m_sURLArgs, 0);
		m_VideoEncoder.open_codec(m_recordFileWapper.get_video_stream_codec());
	}
	m_LogoFilter.init(m_recordFileWapper.get_video_stream_codec());
	if (done)
		return S_OK;
	else
		return E_UNEXPECTED;
...
int XiaozaiAVController::ProcessVideo(AVPacket* &encodeV, AVFrame* &myframe, bool &bNeedFree, AVPacket * mupacket)
{
	//Decoding the video frame
	myframe = m_VideoDecoder.decode_with_scale(mupacket, 0, 0, AV_PIX_FMT_YUV420P);
	if (myframe == NULL) {
		printf("decode_with_scale return error\n");
		return -1;
	}
	int ret = 0;

	AVFrame * theFrame = NULL;
	if (m_LogoFilter.IsInited())
	{
		//Add a logo 
	...
	//Write to a file
	if (m_recordFileWapper.IsOpened() && encodeV) {
		ret = m_recordFileWapper.write_video_frame(av_packet_clone(encodeV));
	}
...
int XiaozaiAVController::ProcessAudio(AVPacket* &encodeV, AVFrame* &myframe, bool &bNeedFree, AVPacket * mupacket)
{
	...
	if (m_AudioDecoder.IsAAC() != 1)
	{
		...
		if (isopen)
		{
			...
			if (m_recordFileWapper.IsOpened() && encodeV)
				ret = m_recordFileWapper.write_audio_frame(av_packet_clone(encodeV));
			
			if (m_rtmpURLWapper.IsOpened() && encodeV)
				ret = m_rtmpURLWapper.write_audio_frame(av_packet_clone(encodeV));
			...
			if (ret > 0)
			{
				...
				{
					if (m_recordFileWapper.IsOpened() && pkt)
						ret = m_recordFileWapper.write_audio_frame(av_packet_clone(pkt));
	...
	//== CODE ADD BY ANYZ =================================================================//
	// Date: 2016/04/18 9:44:54
	// Despcription: 取出塞入的音频包，不做什么操作
	AVFrame* aud = NULL;
	m_RawSamplePool.GetStreamSample(m_AudioStreamIndex, &aud);
	av_frame_unref(aud);
	av_frame_free(&aud);
	//== CODE END =========================================================================//
...
int XiaozaiAVController::Run()
{
	...
	if (ret > 0)
	{
		...
		{
			if (m_recordFileWapper.IsOpened() && pkt)
				ret = m_recordFileWapper.write_video_frame(av_packet_clone(pkt));
	...
	m_recordFileWapper.flush();
	m_rtmpURLWapper.flush();

	return 0;
}

HRESULT STDMETHODCALLTYPE XiaozaiAVController::FillMediaSample(BYTE* pData, LONG lDataLen, BOOL bVideo, REFERENCE_TIME *rtStart, REFERENCE_TIME *rtEnd, LONG *size)
{
	AVFrame* theFrame = NULL;
	HRESULT hres = S_OK;
	BYTE * pBuf = pData;
...
HRESULT STDMETHODCALLTYPE XiaozaiAVController::GetAudioInfo(PWORD nChannels,         // Number of channels
...
HRESULT STDMETHODCALLTYPE XiaozaiAVController::GetVideoInfo(PLONG Width, PLONG Height, DOUBLE* fps)
{
	if (m_DshowDeviceWapper.IsOpened())
	{
...
HRESULT STDMETHODCALLTYPE XiaozaiAVController::StarPlay(BOOL bWithAudio)
{
...
HRESULT STDMETHODCALLTYPE XiaozaiAVController::StopPlay()
{
	//this->Wait(1000);
	this->PostMessageW(XIAOZAI_QUIT, 0, NULL);
	this->Wait(1000);
	this->Terminate();
	...
	if (m_recordFileWapper.IsOpened())
		m_recordFileWapper.CleanUp();
	if (m_rtmpURLWapper.IsOpened())
		m_rtmpURLWapper.CleanUp();

	//m_VideoEncoder.setClosed();
	//m_AudioEncoder.setClosed();
...
HRESULT STDMETHODCALLTYPE XiaozaiAVController::PausePlay()
{
	this->Suspend();
...
HRESULT STDMETHODCALLTYPE XiaozaiAVController::ResumePlay()
{
	this->Resume();
...
HRESULT STDMETHODCALLTYPE XiaozaiAVController::EnableAudio()
{
...
HRESULT STDMETHODCALLTYPE XiaozaiAVController::DisableAudio()
{
	m_bAudioEnabled = FALSE;
	return S_OK;
}

HRESULT STDMETHODCALLTYPE XiaozaiAVController::SetMessageLoopMode(HWND hMsgWnd, HWND hDspWnd, UINT iMsg)
{
	if (m_running)
		return E_FAIL;
...
int XiaozaiAVController::InitD3D(HWND hwnd, unsigned long lWidth, unsigned long lHeight)
{
    HRESULT lRet;
	//== CODE ADD BY ANYZ =================================================================//
	// Date: 2016/04/07 13:28:03
	if (m_pDirect3D9)
		return S_OK;
	//== CODE END =========================================================================//
...
HRESULT STDMETHODCALLTYPE XiaozaiAVController::RenderVideoFrame()
{
    HRESULT lRet;
...
int XiaozaiAVController::CopyCodecCxtProperties()
{
...
HRESULT XiaozaiAVController::ShowCameraProps()
{
	...
	avformat_open_input(&pFormatCtx, "video=HD WebCam", iformat, &options);
